...
function App({ onSaveCard }) {
  ...
  const saveToDatabase = async () => {
    if (!cardBlob || !onSaveCard || saving) return;
    setSaving(true);
    setError(null);
    try {
      await onSaveCard(cardBlob, meta);
      setSaving(false);
    } catch (err) {
            ...
            <div style={{ ...cardStyle, flex:1, minWidth:280 }}>
              ...
              {onSaveCard && (
                <button
                  ...
                  disabled={!cardBlob || saving}
                  style={{
                    width:"100%", marginTop:12, padding:"14px 0", borderRadius:12, border:"none",
                    fontSize:15, fontWeight:700, cursor: cardBlob && !saving ? "pointer" : "default",
                    background: cardBlob && !saving
                      ? "linear-gradient(135deg,#16A34A,#15803D)"
                      : "#CBD5E1",
                    color:"#fff", transition:"all .2s",
                    boxShadow: cardBlob && !saving ? "0 4px 14px rgba(22,163,74,.3)" : "none",
                  }}
                >
